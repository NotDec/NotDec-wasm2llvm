...
struct BreakoutTarget {
  ...
  BreakoutTarget(llvm::BasicBlock &target, std::deque<llvm::PHINode *> phis,
                 std::size_t pos, wabt::BlockDeclaration &sig,
                 wabt::LabelType lty)
...
struct BlockContext {
  ...
  // rvalue reference here, use it by std::move(locals): BlockContext
  // bctx(*function, irBuilder, std::move(locals));
  BlockContext(Context &ctx, llvm::Function &f, llvm::IRBuilder<> &b,
               std::vector<llvm::Value *> &&locals)
      : ctx(ctx), llvmContext(ctx.llvmContext), function(f), irBuilder(b),
        locals(locals), log_level(ctx.opts.LogLevel), type(ctx.llvmContext) {}

  void visitBlock(wabt::LabelType lty, llvm::BasicBlock *entry,
                  llvm::BasicBlock *exit, wabt::BlockDeclaration &decl,
                  wabt::ExprList &exprs);
  void visitControlInsts(llvm::BasicBlock *entry, llvm::BasicBlock *exit,
                         wabt::ExprList &exprs);
  void dispatchExprs(wabt::Expr &expr);
  void unwindStackTo(size_t pos);

  llvm::Instruction *visitBr(wabt::Expr *expr, std::size_t ind,
                             llvm::Value *cond, llvm::BasicBlock *nextBlock);
  ...
  llvm::Value *createLoadLane(llvm::Value *vector, llvm::Value *addr,
                              llvm::Type *ty, uint64_t imm);
  llvm::Value *createStoreLane(llvm::Value *vector, llvm::Value *addr,
                               llvm::Type *ty, uint64_t imm);
  llvm::Value *createExtractLane(llvm::Value *vector, llvm::Type *ty,
                                 uint64_t imm, bool sign);
  llvm::Value *createReplaceLane(llvm::Value *vector, llvm::Value *scalar,
                                 llvm::Type *ty, uint64_t imm);
  llvm::Value *createAllTrue(llvm::Value *vector,
                             llvm::FixedVectorType *vectorType);
  llvm::Value *createSIMDExtend(llvm::Value *vector,
                                llvm::FixedVectorType *vectorType,
                                llvm::FixedVectorType *destType,
                                unsigned int startIdx, bool sign);
  llvm::Value *createSIMDTrunc(llvm::Value *vector,
                               llvm::FixedVectorType *vectorType,
                               llvm::FixedVectorType *destType,
                               llvm::Value *floatMin, bool sign);
  llvm::Value *popStack() {
    assert(stack.size() >= 1);
    llvm::Value *p1 = stack.back();
    stack.pop_back();
    return p1;
  }
};
