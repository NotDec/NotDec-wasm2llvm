...
void BlockContext::visitBlock(wabt::LabelType lty, llvm::BasicBlock *entry,
                              llvm::BasicBlock *exit,
                              wabt::BlockDeclaration &decl,
                              wabt::ExprList &exprs) {
  ...
  if (log_level >= level_debug) {
    std::cerr << "Debug: Visiting Block with label type "
              << labelTypeToString(lty) << std::endl;
  }
  ...
  case LabelType::If:
    ...
    for (wabt::Index i = 0; i < decl.GetNumResults(); i++) {
      llvm::PHINode *phi =
          irBuilder.CreatePHI(convertType(llvmContext, decl.GetResultType(i)),
                              0, exit->getName() + "_" + std::to_string(i));
      phis.push_back(phi);
    }
  case LabelType::Else:
    isBlockLike = true;
  ...
  case LabelType::Loop:
    ...
    for (wabt::Index i = decl.GetNumParams(); i-- > 0;) {
      llvm::PHINode *phi =
          irBuilder.CreatePHI(convertType(llvmContext, decl.GetParamType(i)), 0,
                              entry->getName() + "_" + std::to_string(i));
      phis.push_front(phi);
      // 给phi赋值，然后用Phi替换栈上值
  ...
  default:
    std::cerr << __FILE__ << ":" << __LINE__ << ": "
              << "Error: unexpected LabelType: " << labelTypeToString(lty)
              << std::endl;
    std::abort();
  }
  // 标记当前栈的位置，结束的时候先读取返回值然后再弹出到这个位置。
  std::size_t stack_pos = lty == LabelType::Func
                              ? stack.size()
                              : stack.size() - decl.GetNumParams();
  ...
  if (irBuilder.GetInsertBlock() !=
      nullptr) { // not unreachable, handle implicit return
    if (stack.size() != (stack_pos + decl.GetNumResults())) {
      std::cerr << __FILE__ << ":" << __LINE__ << ": "
                << "Error: Not enough value on stack" << std::endl;
      std::cerr << decl.GetNumResults() << " expected, but got "
                << (stack.size() - stack_pos) << std::endl;
      std::abort();
    }
    ...
    if (isBlockLike) {
      for (auto it = phis.rbegin(); it != phis.rend(); ++it) {
        (*it)->addIncoming(stack.back(), irBuilder.GetInsertBlock());
        stack.pop_back();
      }
      if (lty != LabelType::If) {
        for (auto phi : phis) {
    ...
    if (stack.size() > stack_pos) {
      if (log_level >= level_info) {
        std::cerr << "Info: Discard " << stack.size() - stack_pos
                  << " unreachable values." << std::endl;
      }
    ...
    if (stack.size() < stack_pos) {
      std::cerr << __FILE__ << ":" << __LINE__ << ": "
                << "Error: Not enough values!" << std::endl;
      std::abort();
    }
    // push 返回值
    for (auto phi : phis) {
...
void BlockContext::unwindStackTo(std::size_t stack_pos) {
  if (stack.size() > stack_pos) {
    if (log_level >= level_debug) {
      std::cerr << "Debug: Unwind " << stack.size() - stack_pos << " values."
                << std::endl;
    }
    while (stack.size() > stack_pos) {
      stack.pop_back();
  ...
  for (wabt::Expr &expr : exprs) { // 遍历每个指令的主要循环
    ...
    if (irBuilder.GetInsertBlock() == nullptr) { // unreachable place after br
      if (log_level >= level_warning) {
        if (expr.type() != wabt::ExprType::Unreachable) {
          std::cerr << "Warning: Skipping unreachable expr "
                    << wabt::GetExprTypeName(expr);
          if (expr.loc.line != 0) {
    ...
    case wabt::ExprType::Block: {
      ...
      // 创建新的基本块作为entry，exit
      // BasicBlock* entryBlock = llvm::BasicBlock::Create(llvmContext,
      // "blkentry", &function);
      BasicBlock *exitBlock =
          llvm::BasicBlock::Create(llvmContext, "blk_exit", &function);
      visitBlock(wabt::LabelType::Block, entry, exitBlock, e->block.decl,
                 e->block.exprs);
      entry = exitBlock;
      irBuilder.SetInsertPoint(entry);
      break;
    ...
    case wabt::ExprType::Loop: {
      ...
      BasicBlock *entryBlock =
          llvm::BasicBlock::Create(llvmContext, "loop_entry", &function);
      irBuilder.CreateBr(entryBlock);
      entry = entryBlock;
      irBuilder.SetInsertPoint(entry);
      visitBlock(wabt::LabelType::Loop, entryBlock, exit, e->block.decl,
                 e->block.exprs);
    ...
    case wabt::ExprType::If: {
      ...
      // convert to i1
      p1 = irBuilder.CreateICmpNE(p1, ConstantInt::getNullValue(p1->getType()),
                                  "brif_val");
      BasicBlock *ifBlock = BasicBlock::Create(llvmContext, "ifblk", &function);
      BasicBlock *elseBlock =
          BasicBlock::Create(llvmContext, "elseblk", &function);
      BasicBlock *exitBlock =
          BasicBlock::Create(llvmContext, "ifblk_exit", &function);
      irBuilder.CreateCondBr(p1, ifBlock, elseBlock);

      entry = ifBlock;
      irBuilder.SetInsertPoint(entry);
      visitBlock(wabt::LabelType::If, entry, exitBlock, e->true_.decl,
                 e->true_.exprs);
      // duplicate parameter
      ...
      entry = elseBlock;
      irBuilder.SetInsertPoint(entry);
      visitBlock(wabt::LabelType::Else, entry, exitBlock, e->true_.decl,
                 e->false_);
      entry = exitBlock;
      irBuilder.SetInsertPoint(entry);
      break;
    ...
    case wabt::ExprType::BrIf: {
      ...
      BasicBlock *nextBlock =
          llvm::BasicBlock::Create(llvmContext, "brif_next", &function);
    ...
    case wabt::ExprType::BrTable: {
      ...
      SwitchInst *si = cast<SwitchInst>(
          visitBr(brt, blockStack.size() - defInd, p1, nullptr));
      // 其他target
      for (wabt::Index i = 0; i < brt->targets.size(); i++) {
        ...
        si->addCase(ConstantInt::get(Type::getInt32Ty(llvmContext), i),
                    &bt.target);
        auto stackIt = stack.rbegin();
        for (auto it = bt.phis.rbegin(); it != bt.phis.rend();
             ++it, ++stackIt) {
          if (stackIt == stack.rend()) {
            std::cerr << __FILE__ << ":" << __LINE__ << ": "
                      << "Error: Not enough value for br_table ind " << i
                      << " level " << brt->targets.at(i).index() << std::endl;
            std::cerr << "Note: Jump target lable " << labelTypeToString(bt.lty)
                      << " param num " << bt.sig.GetNumParams()
                      << " result num " << bt.sig.GetNumResults() << std::endl;
            std::abort();
          }
...
llvm::Instruction *BlockContext::visitBr(wabt::Expr *expr, std::size_t ind,
                                         llvm::Value *cond,
  ...
  if (cond == nullptr && nextBlock == nullptr) {
    assert(expr->type() == wabt::ExprType::Br ||
           expr->type() == wabt::ExprType::Return);
    ...
    std::cerr << __FILE__ << ":" << __LINE__ << ": "
              << "Error: Unexpected call to BlockContext::visitBr: "
              << std::endl;
    std::abort();
  }
  return ret;
...
wabt::Block &getBlock(wabt::Expr &expr) {
  ...
  default:
    std::cerr << __FILE__ << ":" << __LINE__ << ": "
              << "Error: Unsupported expr type for `getBlock`: "
              << GetExprTypeName(expr) << std::endl;
    std::abort();
  }
}
