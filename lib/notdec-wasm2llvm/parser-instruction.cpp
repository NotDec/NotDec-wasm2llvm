...
void BlockContext::dispatchExprs(wabt::Expr &expr) {
  ...
  case ExprType::MemoryGrow:
    std::cerr << __FILE__ << ":" << __LINE__ << ": "
              << "Warning: dummy implementation for " << GetExprTypeName(expr)
              << std::endl;
    stack.pop_back();
    stack.push_back(
  ...
  case ExprType::SimdStoreLane:
    ...
    break;
  case ExprType::
      SimdShuffleOp: { // https://discourse.llvm.org/t/createshufflevector/67947/2
    ...
    if (shuffle->opcode == Opcode::I8X16Shuffle) {
      ...
      for (unsigned int i = 0; i < 16; i++)
        mask[i] = imm.u8(i);
      stack.push_back(
          irBuilder.CreateShuffleVector(a, b, llvm::ArrayRef<int>(mask, 16)));
    } else
      std::cerr << __FILE__ << ":" << __LINE__ << ": "
                << "Error: Unsupported Opcode: " << shuffle->opcode.GetName()
                << std::endl;
  } break;

  default:
    std::cerr << __FILE__ << ":" << __LINE__ << ": "
              << "Error: Unsupported expr type: " << GetExprTypeName(expr)
              << std::endl;
    // really abort?
    std::abort();
  }
...
void BlockContext::visitSelectExpr(wabt::SelectExpr *expr) {
  ...
  Value *cond = popStack();
  // convert to i1
  cond = irBuilder.CreateICmpNE(
      cond, ConstantInt::getNullValue(cond->getType()), "select_cond");
  Value *val2 = popStack();
  Value *val1 = popStack();
...
void BlockContext::visitLoadInst(wabt::LoadExpr *expr) {
  ...
  default:
    std::cerr << __FILE__ << ":" << __LINE__ << ": "
              << "Error: Unsupported Opcode: " << expr->opcode.GetName()
              << std::endl;
    break;
  }
  assert(targetType != nullptr);
  ...
  case wabt::Opcode::I64Load16S:
  case wabt::Opcode::I64Load32S:
    result = irBuilder.CreateSExt(
        result, convertType(llvmContext, expr->opcode.GetResultType()));
    break;
  case wabt::Opcode::I32Load16U:
  case wabt::Opcode::I64Load16U:
  case wabt::Opcode::I32Load8U:
  ...
  case wabt::Opcode::V128Load32Zero:
    result = irBuilder.CreateInsertElement(
        ConstantVector::getNullValue(type.i32x4Type), result, (uint64_t)(0));
    break;
  case wabt::Opcode::V128Load64Zero:
    result = irBuilder.CreateInsertElement(
        ConstantVector::getNullValue(type.i64x2Type), result, (uint64_t)(0));
    break;
  case wabt::Opcode::V128Load8X8S:
...
void BlockContext::visitUnaryInst(wabt::UnaryExpr *expr) {
  ...
  case wabt::Opcode::I8X16Bitmask: {
    ...
    Value *i1x16Mask = irBuilder.CreateICmpSLT(
  ...
  case wabt::Opcode::I16X8Bitmask: {
    ...
    Value *i1x8Mask = irBuilder.CreateICmpSLT(
        i8x16Operand, ConstantVector::getNullValue(type.i16x8Type));
    ret = irBuilder.CreateZExt(irBuilder.CreateBitCast(i1x8Mask, type.i8Type),
                               type.i32Type);
  } break;
  case wabt::Opcode::I32X4Bitmask: {
    Value *i32x4Operand = irBuilder.CreateBitCast(p1, type.i32x4Type);
    Value *i1x4Mask = irBuilder.CreateICmpSLT(
        i32x4Operand, ConstantVector::getNullValue(type.i32x4Type));
    ret = irBuilder.CreateZExt(
        irBuilder.CreateBitCast(i1x4Mask, IntegerType::get(llvmContext, 4)),
        type.i32Type);
  } break;
  case wabt::Opcode::I64X2Bitmask: {
    Value *i64x2Operand = irBuilder.CreateBitCast(p1, type.i64x2Type);
    Value *i1x2Mask = irBuilder.CreateICmpSLT(
        i64x2Operand, ConstantVector::getNullValue(type.i64x2Type));
    ret = irBuilder.CreateZExt(
        irBuilder.CreateBitCast(i1x2Mask, IntegerType::get(llvmContext, 2)),
        type.i32Type);
  } break;
  ...
  case wabt::Opcode::V128AnyTrue: {
    ConstantInt *zero =
        ConstantInt::get(Type::getInt64Ty(irBuilder.getContext()), 0);
    Value *boolResult = irBuilder.CreateOr(
        irBuilder.CreateICmpNE(irBuilder.CreateExtractElement(p1, uint64_t(0)),
                               zero),
        irBuilder.CreateICmpNE(irBuilder.CreateExtractElement(p1, uint64_t(1)),
                               zero));
    ret = irBuilder.CreateZExt(boolResult,
                               Type::getInt32Ty(irBuilder.getContext()));
    break;
  }
  // SIMD Splat
  case wabt::Opcode::I8X16Splat:
    ret = irBuilder.CreateVectorSplat(
        16, irBuilder.CreateTrunc(p1, Type::getInt8Ty(llvmContext)));
    break;
  case wabt::Opcode::I16X8Splat:
    ret = irBuilder.CreateVectorSplat(
        8, irBuilder.CreateTrunc(p1, Type::getInt16Ty(llvmContext)));
    break;
  ...
  case wabt::Opcode::F32X4DemoteF64X2Zero: {
    ...
    ret = irBuilder.CreateInsertElement(
        ret, irBuilder.CreateExtractElement(vec, uint64_t(0)), uint64_t(0));
    ret = irBuilder.CreateInsertElement(
        ret, irBuilder.CreateExtractElement(vec, uint64_t(1)), uint64_t(1));
  } break;
  case wabt::Opcode::F64X2PromoteLowF32X4: {
    Value *vec = irBuilder.CreateBitCast(p1, type.f32x4Type);
    vec = irBuilder.CreateShuffleVector(vec, UndefValue::get(type.f32x4Type),
                                        (ArrayRef<int>){0, 1});
  ...
  case wabt::Opcode::I16X8ExtaddPairwiseI8X16S: {
    ...
    Value *vec1 = irBuilder.CreateShuffleVector(
        vec, undef, (ArrayRef<int>){0, 2, 4, 6, 8, 10, 12, 14});
    Value *vec2 = irBuilder.CreateShuffleVector(
        vec, undef, (ArrayRef<int>){1, 3, 5, 7, 9, 11, 13, 15});
  ...
  case wabt::Opcode::I32X4ExtaddPairwiseI16X8S: {
    ...
    Value *vec1 =
        irBuilder.CreateShuffleVector(vec, undef, (ArrayRef<int>){0, 2, 4, 6});
    Value *vec2 =
        irBuilder.CreateShuffleVector(vec, undef, (ArrayRef<int>){1, 3, 5, 7});
  ...
  case wabt::Opcode::F32X4ConvertI32X4S:
    EMIT_SIMD_UNARY_OP(type.f32x4Type,
                       irBuilder.CreateSIToFP(v, type.f32x4Type));
  case wabt::Opcode::F32X4ConvertI32X4U:
    EMIT_SIMD_UNARY_OP(type.f32x4Type,
                       irBuilder.CreateUIToFP(v, type.f32x4Type));
...
llvm::Value *BlockContext::createSIMDExtend(llvm::Value *vector,
                                            llvm::FixedVectorType *vectorType,
                                            llvm::FixedVectorType *destType,
                                            unsigned int startIdx, bool sign) {
  using namespace llvm;
  vector = irBuilder.CreateBitCast(vector, vectorType);
  FixedVectorType *halfVectorType =
      FixedVectorType::getHalfElementsVectorType(vectorType);
...
#define EMIT_SIMD_BINARY_OP(llvmType, emitCode)                                \
  ...
  case wabt::Opcode::I8X16AddSatU: {
    ...
    ret =
        irBuilder.CreateSelect(irBuilder.CreateICmpUGT(left, add),
                               Constant::getAllOnesValue(left->getType()), add);
  } break;
...
#define EMIT_SIMD_SHIFT_OP(llvmType, emitCode)                                 \
  {                                                                            \
    ...
    llvm::Value *mask = irBuilder.CreateAnd(                                   \
        p1, llvm::ConstantInt::get(p1->getType(), numBits - 1));               \
  ...
  case wabt::Opcode::I8X16AvgrU: {
    ...
    ret = irBuilder.CreateTrunc(
        irBuilder.CreateLShr(
            irBuilder.CreateAdd(irBuilder.CreateAdd(leftZExt, rightZExt),
                                oneZExt),
            oneZExt),
        type.i8x16Type);
  } break;
  ...
  case wabt::Opcode::I8X16MinS:
    EMIT_SIMD_BINARY_OP(type.i8x16Type,
                        irBuilder.CreateSelect(
                            irBuilder.CreateICmpSLT(left, right), left, right));
    break;
  case wabt::Opcode::I8X16MinU:
    EMIT_SIMD_BINARY_OP(type.i8x16Type,
                        irBuilder.CreateSelect(
                            irBuilder.CreateICmpULT(left, right), left, right));
    break;
  ...
  case wabt::Opcode::F32X4PMin:
    EMIT_SIMD_BINARY_OP(
        type.f32x4Type,
        irBuilder.CreateSelect(
            irBuilder.CreateFCmp(CmpInst::FCMP_OLT, right, left), right, left));
    break;
  ...
  case wabt::Opcode::I8X16MaxS:
    EMIT_SIMD_BINARY_OP(type.i8x16Type,
                        irBuilder.CreateSelect(
                            irBuilder.CreateICmpSLT(left, right), right, left));
    break;
  case wabt::Opcode::I8X16MaxU:
    EMIT_SIMD_BINARY_OP(type.i8x16Type,
                        irBuilder.CreateSelect(
                            irBuilder.CreateICmpULT(left, right), right, left));
    break;
  ...
  case wabt::Opcode::F32X4PMax:
    EMIT_SIMD_BINARY_OP(
        type.f32x4Type,
        irBuilder.CreateSelect(
            irBuilder.CreateFCmp(CmpInst::FCMP_OLT, left, right), right, left));
    break;
  ...
  case wabt::Opcode::I16X8Q15mulrSatS: {
    ...
    Value *sum = irBuilder.CreateAdd(
        product, irBuilder.CreateVectorSplat(
                     8, ConstantInt::get(llvmContext,
                                         APInt(32, (uint64_t)0x4000, false))));
    Value *shift = irBuilder.CreateAShr(sum, 15);
    Value *minSplat = irBuilder.CreateVectorSplat(
        8, ConstantInt::get(llvmContext, APInt(32, (int64_t)INT16_MIN, false)));
    Value *maxSplat = irBuilder.CreateVectorSplat(
        8, ConstantInt::get(llvmContext, APInt(32, (int64_t)INT16_MAX, false)));
    Value *saturate = irBuilder.CreateSelect(
        irBuilder.CreateICmpSGT(shift, maxSplat), maxSplat,
        irBuilder.CreateSelect(irBuilder.CreateICmpSLT(shift, minSplat),
                               minSplat, shift));
...
void BlockContext::visitCallInst(wabt::CallExpr *expr) {
  ...
  Value *ret =
      irBuilder.CreateCall(target->getFunctionType(), target, callArgs);
  if (wfunc->GetNumResults() != 0) {
    stack.push_back(ret);
  }
}

llvm::Value *BlockContext::createAllTrue(llvm::Value *vector,
                                         llvm::FixedVectorType *vectorType) {
  using namespace llvm;
  vector = irBuilder.CreateBitCast(vector, vectorType);
  ...
  Constant *zero = ConstantInt::get(
      vectorType->getScalarType(), APInt(vectorType->getScalarSizeInBits(), 0));
...
llvm::Constant *visitConst(llvm::LLVMContext &llvmContext,
                           const wabt::Const &const_) {
  using namespace wabt;
  uint64_t data[2];
  switch (const_.type()) {
  case Type::I32:
    return llvm::ConstantInt::get(convertType(llvmContext, const_.type()),
                                  const_.u32(), false);
  case Type::I64:
    return llvm::ConstantInt::get(convertType(llvmContext, const_.type()),
                                  const_.u64(), false);
  case Type::F32:
    return llvm::ConstantFP::get(convertType(llvmContext, const_.type()),
                                 llvm::APFloat(ieee_float(const_.f32_bits())));
  case Type::F64:
    return llvm::ConstantFP::get(convertType(llvmContext, const_.type()),
                                 llvm::APFloat(ieee_double(const_.f64_bits())));
  case Type::V128:
    data[0] = const_.vec128().u64(0);
    data[1] = const_.vec128().u64(1);
    return llvm::ConstantInt::get(
        convertType(llvmContext, const_.type()),
        llvm::APInt(128, llvm::ArrayRef<uint64_t>(data, 2)));
  default:
    std::cerr << __FILE__ << ":" << __LINE__ << ": "
              << "Error: InitExpr type unknown: " << const_.type().GetName()
              << std::endl;
    std::abort();
  }
}

llvm::Value *BlockContext::createExtractLane(llvm::Value *vector,
                                             llvm::Type *ty, uint64_t imm,
                                             bool sign) {
  using namespace llvm;
  Value *ret =
      irBuilder.CreateExtractElement(irBuilder.CreateBitCast(vector, ty), imm);
  if (sign)
...
llvm::Value *BlockContext::createLoadLane(llvm::Value *vector,
                                          llvm::Value *addr, llvm::Type *ty,
                                          uint64_t imm) {
  using namespace llvm;
...
llvm::Value *BlockContext::createStoreLane(llvm::Value *vector,
                                           llvm::Value *addr, llvm::Type *ty,
                                           uint64_t imm) {
  using namespace llvm;
  vector = irBuilder.CreateBitCast(vector, ty);
